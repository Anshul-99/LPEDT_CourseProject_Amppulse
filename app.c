...
SL_WEAK void app_init(void)
{
  ...
  sl_power_manager_add_em_requirement(LOWEST_ENERGY_MODE);
    ...
    init_osc(cmuOsc_ULFRCO, cmuSelect_ULFRCO);
    init_letimer(LOWEST_ENERGY_MODE);
  #else
    init_osc(cmuOsc_LFXO, cmuSelect_LFXO);
    init_letimer(LOWEST_ENERGY_MODE);
  #endif
    ...
    //init_i2c(); //Initialize I2C It gets initialized in the write/read functions
...
//   sensor_state_machine(evt);    // put this code in scheduler.c/.h
   ...
   max30102_state_machine(evt);

#elif (DEVICE_IS_BLE_SERVER == 0)

   /* Helps the client to discover the services, characteristics provided by the server.
    * It also helps the client read temperature from the server characteristic, respond to
    * indications.
    */
