...
/* Number of ticks given as input to the soft timer to generate a period of 1s */
#define TICKS_PER_SECOND (32768)
#define TIMER_MODE (0) /*Repeating mode */
#define TIMER_HANDLE (0) /* Timer handle used. */ //TODO:
...
void displayInit()
{
    ...
    toggle_power(Sensor_ON); /* switch on Si7021 sensor */
    ...
    //           set up a 1 second repeating soft timer and uncomment the following lines
	  ...
	  timer_response = sl_bt_system_set_soft_timer(TICKS_PER_SECOND, TIMER_HANDLE, TIMER_MODE);
	  if (timer_response != SL_STATUS_OK) {
	      LOG_ERROR("soft timer returned error %d\n\r", timer_response);
     }
