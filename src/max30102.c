...
#define MAX30102_DEVICE_ADDRESS          (0x57) //7-bit I2C Address
...
{
  return (readRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_INTSTAT1));
}
uint8_t getINT2(void)
{
  return (readRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_INTSTAT2));
}
...
void softReset(void)
{
  ...
  // Timeout after 100ms
  unsigned long startTime = letimerMilliseconds();
  while (letimerMilliseconds() - startTime < 100)
  {
    uint8_t response = readRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_MODECONFIG);
...
{
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_LED1_PULSEAMP, amplitude);
}

void setPulseAmplitudeIR(uint8_t amplitude)
{
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_LED2_PULSEAMP, amplitude);
}

void setPulseAmplitudeGreen(uint8_t amplitude)
{
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_LED3_PULSEAMP, amplitude);
}

void setPulseAmplitudeProximity(uint8_t amplitude)
{
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_LED_PROX_AMP, amplitude);
}
...
void setProximityThreshold(uint8_t threshMSB)
{
  ...
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_PROXINTTHRESH, threshMSB);
}
...
{
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_MULTILEDCONFIG1, 0);
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_MULTILEDCONFIG2, 0);
}
...
void clearFIFO(void)
{
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_FIFOWRITEPTR, 0);
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_FIFOOVERFLOW, 0);
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_FIFOREADPTR, 0);
}
...
{
  return (readRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_FIFOWRITEPTR));
}

//Read the FIFO Read Pointer
uint8_t getReadPointer(void)
{
  return (readRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_FIFOREADPTR));
}
...
float readTemperature()
{
  ...
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_DIETEMPCONFIG, 0x01);
  ...
  while (letimerMilliseconds() - startTime < 100)
  {
    //uint8_t response = readRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_DIETEMPCONFIG); //Original way
    //if ((response & 0x01) == 0) break; //We're done!

  //Check to see if DIE_TEMP_RDY interrupt is set
  uint8_t response = readRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_INTSTAT2);
    if ((response & MAX30105_INT_DIE_TEMP_RDY_ENABLE) > 0) break; //We're done!
    //delay(1); //Let's not over burden the I2C bus
  }
  //TODO How do we want to fail? With what type of error?
  //? if(letimerMilliseconds() - startTime >= 100) return(-999.0);

  // Step 2: Read die temperature register (integer)
  int8_t tempInt = readRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_DIETEMPINT);
  uint8_t tempFrac = readRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_DIETEMPFRAC); //Causes the clearing of the DIE_TEMP_RDY interrupt
...
{
  writeRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_PROXINTTHRESH, val);
}
...
uint8_t readPartID()
{
  return readRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_PARTID);
}

void readRevisionID()
{
  revisionID = readRegister8(MAX30102_DEVICE_ADDRESS, MAX30105_REVISIONID);
}
...
//    _i2cPort->endTransmission();
    ...
    while (uint8_tsLeftToRead > 0)
    {
      ...
      while (toGet > 0)
      {
        ...
        temp[3] = 0;
        I2C_write_read(0x57, 0x07, rdata, 9);
        temp[2] = rdata[0];
        temp[1] = rdata[1];
        temp[0] = rdata[2];
        ...
        if (activeLEDs > 1)
        {
          ...
          temp[3] = 0;
          //I2C_write_read(0x57, 0x07, rdata, 3);
          temp[2] = rdata[3];
          temp[1] = rdata[4];
          temp[0] = rdata[5];
        ...
        if (activeLEDs > 2)
        {
          ...
          //uint8_t rdataGreen[3];

          temp[3] = 0;
          //I2C_write_read(0x57, 0x07, rdataGreen, 3);
          temp[2] = rdata[6];
          temp[1] = rdata[7];
          temp[0] = rdata[8];
...
{
  uint32_t markTime = letimerMilliseconds();

  while(1)
  {
  if(letimerMilliseconds() - markTime > maxTimeToCheck) return(false);

  if(check() == true) //We found new data!
  {
    //Serial.print("Going to check");
    return(true);
  }
...
void bitMask(uint8_t reg, uint8_t mask, uint8_t thing)
{
  ...
  uint8_t originalContents = readRegister8(MAX30102_DEVICE_ADDRESS, reg);

  // Zero-out the portions of the register we're interested in
  originalContents = originalContents & mask;

  // Change contents
  writeRegister8(MAX30102_DEVICE_ADDRESS, reg, originalContents | thing);
}
...
uint8_t readRegister8(uint8_t address, uint8_t reg)
{
  ...
  uint8_t rdata;
  I2C_write_read(address, reg, &rdata, 1);
  return rdata;
...
{
  I2C_write_write(address, reg, &value, 1);
}
